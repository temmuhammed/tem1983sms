...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/temmuhammed/tem1983sms/archive/dev.zip"
        dir_name = "tem1983sms-dev"
    else:
        zip_url = "https://github.com/temmuhammed/tem1983sms/archive/master.zip"
        dir_name = "tem1983sms-master"
    ...
    if success:
        mesgdcrt.SuccessMessage("tem1983sms was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update tem1983sms.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/temmuhammed/tem1983sms.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/temmuhammed/tem1983sms.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://github.com/temmuhammed/tem1983sms/master/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("tem1983sms is up-to-date")
        mesgdcrt.GeneralMessage("Starting tem1983sms")


def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/temmuhammed/tem1983sms/raw/dev/.notify"
        else:
            url = "https://github.com/temmuhammed/tem1983sms/raw/master/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("tem1983sms was created by Muhammed")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("tem1983sms will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['Muhammed', 't0xic0der', 'scpketer', 'Stefan']
...
description = """tem1983sms - Your Friendly Spammer Application

tem1983sms can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

tem1983sms is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by Muhammed !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start tem1983sms with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start tem1983sms with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start tem1983sms with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update tem1983sms")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current tem1983sms contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current tem1983sms version")
